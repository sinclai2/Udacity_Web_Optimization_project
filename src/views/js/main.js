...
function updatePositions() {
  ...
  // move scrollTop outside of the loop. less thing to do each loop
  var scrollTop = document.documentElement.scrollTop || document.body.scrollTop;
  for (var i = 0; i < items.length; i++) {
    // document.body.scrollTop is no longer supported in Chrome.
    var phase = Math.sin((scrollTop / 1250) + (i % 5));
    // using translateX instead of left to move the pizza
    items[i].style.transform = 'translateX('+ ((items[i].basicLeft + 100 * phase ) - 800) + 'px)';
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // limit the amount of animated pizzas to 30
  var totalPizzas = 30;
